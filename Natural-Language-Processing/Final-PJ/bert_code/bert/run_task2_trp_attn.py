...
from utils_task2_trp_attn import *
from model.bertmodel_attn import Task2Model
...
def evaluate(args, model, tokenizer, test=False):
    ...
    eval_dataset = load_examples_and_cache_features(data_dir=args.data_dir, data_name=args.data_name,
                                                    tokenizer=tokenizer, max_seq_length=args.max_seq_length, max_trp_length=args.max_trp_length,
                                                    trp_num = args.trp_num, dev=not test, test=test)
...
def main():
    ...
    parser.add_argument("--data_name", default="openIE", type=str, help="Prefix of tsv files. If default, then train.tsv et.") # openIE
    ...
    parser.add_argument("--max_seq_length", default=95, type=int, help="The maximum total input sequence length after tokenization. Sequences longer than this will be truncated, sequences shorter will be padded.")
    parser.add_argument("--max_trp_length", default=30, type=int, help="The maximum total input sequence length after tokenization. Sequences longer than this will be truncated, sequences shorter will be padded.")
    parser.add_argument("--trp_num", default=4, type=int, help="Maximum number of triples in one answer")
    ...
    parser.add_argument("--trp_attn", default=False)
    ...
    if args.eval_step is None:
        train_dataset = load_examples_and_cache_features(data_dir=args.data_dir, data_name=args.data_name, tokenizer=tokenizer, max_seq_length=args.max_seq_length, max_trp_length=args.max_trp_length, trp_num=args.trp_num)
