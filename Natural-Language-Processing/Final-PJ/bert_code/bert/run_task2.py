...
from utils_task2 import *
from bertmodel import BertForTask2
...
def main():
    ...
    # File directory
    parser.add_argument("--data_dir", default="./dataset/task2/", type=str, help="Input datadir. Should contain the .tsv files for the task.")
    parser.add_argument("--data_name", default="_ex", type=str, help="Prefix of tsv files. If default, then train.tsv et.")
    ...
    parser.add_argument("--max_seq_length", default=110, type=int, help="The maximum total input sequence length after tokenization. Sequences longer than this will be truncated, sequences shorter will be padded.")
    ...
    model = BertForTask2.from_pretrained("./model/pretrained_bert/bert-base-uncased/bert-base-uncased-pytorch_model.bin", config=config)
    model.to(device)
    ...
    if args.eval_step is None:
        train_dataset = load_examples_and_cache_features(data_dir=args.data_dir, data_name=args.data_name, tokenizer=tokenizer, max_seq_length=args.max_seq_length)
    ...
    if args.do_test:
        ...
        model_eval = BertForTask2.from_pretrained(checkpoint, config=config)
