...
class Preprocess4Seq2seq(Pipeline):
	...
	def __call__(self, instance):
			...
			if self.mode == "s2s":
				if self.s2s_add_segment:
					...
					segment_ids = [4] * (len(tokens_a)+2) + \
						[5]*(len(tokens_b)+1)  # SEQ2SEQ used this
			else:
		...
		if self.pos_shift:
			...
			for i, tk in enumerate(tokens):
				...
				if (i >= len(tokens_a)+2) and (tk != '[CLS]'):
					...
					cand_pos.append(i) # Seq2Seq not use
				else:
					special_pos.add(i)
			shuffle(cand_pos)
			...
			for pos in cand_pos:
				...
				if (self.skipgram_prb > 0) and (self.skipgram_size >= 2) and (rand() < self.skipgram_prb):
					...
					if self.mask_whole_word:
						...
						st_pos, end_pos = pos, pos + 1  # Seq2Seq use
		...
		if self.num_qkv > 1:
			...
			mask_qkv = None  # Seq2Seq use

		input_mask = torch.zeros(self.max_len, self.max_len, dtype=torch.long).to(1)
		if self.mode == "s2s":
