...
def scatter_imbalance(inputs, target_gpus, dim=0):
    ...
    """
    def scatter_map(obj):
        if isinstance(obj, torch.Tensor) or isinstance(obj, list):
            # print("our",obj.size(dim))
            if (len(target_gpus) == 4) and (obj.size(dim) == 4):
                return Scatter.apply(target_gpus, (1, 1, 1, 1), dim, obj)
            if (len(target_gpus) == 4) and (obj.size(dim) == 8):
                return Scatter.apply(target_gpus, (1, 2, 2, 3), dim, obj)
            if (len(target_gpus) == 4) and (obj.size(dim) == 24):
                return Scatter.apply(target_gpus, (2, 7, 7, 8), dim, obj)
            if (len(target_gpus) == 4) and (obj.size(dim) == 12):
                return Scatter.apply(target_gpus, (1, 3, 4, 4), dim, obj)
            if (len(target_gpus) == 4) and (obj.size(dim) == 36):
                return Scatter.apply(target_gpus, (6, 10, 10, 10), dim, obj)
            if (len(target_gpus) == 4) and (obj.size(dim) == 16):
                return Scatter.apply(target_gpus, (2, 4, 5, 5), dim, obj)
            if (len(target_gpus) == 4) and (obj.size(dim) == 48):
                return Scatter.apply(target_gpus, (9, 13, 13, 13), dim, obj)
