...
# spark设置
conf = (SparkConf()
    .setMaster('spark://10.192.7.161:7077') # 完全分布式的Master的ip
    .set("spark.local.ip","10.192.7.116")   # 本机的ip
    .set("spark.driver.host","10.192.7.116")
    .setAppName('Bigdata_Spark_Cluster')    # 设定程序名
    .set('spark.executor.memory', '150g')   # 设定内存大小
    .set('spark.driver.memory', '150g')
    .set("spark.executor.cores", '2')   # 设定核的数量
...
# 获取master信息，output：'spark://10.192.7.161:7077'

sc = SparkContext(conf = conf)  # 启动集群
log = sc.textFile("hdfs://10.192.7.161:9000/enwiki.xml")  # 读取预先存储在hdfs上的wiki数据（72G）
...
def topDocsForTerm(term):
    ...
    title_in = data.filter(lambda line: term in line[2]).map(lambda line: line[0])  # 只保留title中含有该关键词的文章index
    title_in = np.array(title_in.collect()).tolist()
    final_index = [i for i in indeces if i in title_in] # 取交集
    return list(zip(titles[final_index[:10]], final_index[indeces])) # 返回前十篇相关的文章，形如 [title, 相关度]

# # 进行测试
# print(topDocsForTerm('History'))
# print('----')
# print(topDocsForTerm('Geography'))Z